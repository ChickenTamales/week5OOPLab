...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		// These exercises are intended to be coded in order 1 through 5.
			...
			Card card = new Card("Two", "Hearts", 2);//excited because I could do this part on my own :)
		    ...
		    //deck.describe();//wow! omg this is awesome. :)
			...
			//deck.describe();//wow this is getting fun thank God! haha
		...
		public static Map<String, List<Card>>createGame (int numOfPlayers){
			...
			Deck deck = new Deck();
			deck.shuffle();//distinct instantiation of a deck because it's inside of this method
			System.out.println("Deck has " + deck.getCards().size() + "cards!");//"Deck has (number of) cards!"
			
			for (int i = 1; i < numOfPlayers; i++){//"increment our index" I like that way of saying it
				List<Card> playerList = new ArrayList<>();
				String playerName = "Player " + i;//concatenate our index
				finalGameBoard.put(playerName, playerList);//put these players into our map
			
			
		}
		
			for (int i = 0; i < 52/numOfPlayers; i++){//as long as the index is less than 52 divided by the number of players (dealing a hand)
